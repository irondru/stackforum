export const PENDING = 0x001
export const SUCCESS = 0x002
export const ERROR = 0x004
export const REQUEST_STATUSES = PENDING + SUCCESS + ERROR
export const GET_TOPICS = 0x008
export const GET_TOPIC = 0x010
export const USER_REQUEST = 0x020
export const QUESTION_REQUEST = 0x040
export const ANSWER_REQUEST = 0x080
export const INITIAL_EDIT = 0x100
export const CREATE = 0x200
export const UPDATE = 0x400
export const DESTROY = 0x800
export const ACTIONS = 0xFFFFFFF8
